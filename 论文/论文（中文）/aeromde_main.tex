...
\begin{figure}[!t]
  ...
  \caption{准静态 UAV 边缘网络中的协同 MoE 推理场景。地面任务通过空地链路接入 UAV，资源异构 UAV 在给定单跳无线拓扑下部署并执行推理模块或专家步骤；当相邻步骤位于不同 UAV 时，系统仅通过可用 UAV-UAV 单跳链路传输中间特征，不可用链路不参与调度，最终将推理结果返回至基站或应急指挥中心。}
...
\begin{figure}[!t]
  ...
  \begin{tikzpicture}[
    ...
    \node[stage, below=of s2] (s3) {
      ...
      计算 $\mathrm{BaseScore}(e,u)=\alpha A(e,u)+\beta B(e,u)+\gamma C(e,u)-\mu W_e$。
    };
...
为便于复现实验，表~\ref{tab:simulation_parameters} 总结了默认仿真场景和无线信道参数，
表~\ref{tab:task_expert_parameters} 给出了任务与专家池设置，表~\ref{tab:algorithm_sweep_parameters}
列出了算法超参数以及敏感性、扩展性实验中的参数扫描范围。
...
\begin{table}[!t]
  ...
  \begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.34\columnwidth}
    ...
    Base positions & $(60,60)$, $(180,60)$, $(60,180)$, $(180,180)$ m \\
    ...
    Communication mode & Ground--UAV access and single-hop UAV--UAV links \\
    Reference gain $\beta_0$ & 1 \\
    Path-loss exponent & 2.5 \\
    Noise density $N_0$ & $10^{-12}$ W/Hz \\
    Bandwidth $B$ & 20 MHz \\
    SNR threshold $\rho_{\mathrm{th}}$ & 0.5 \\
...
\begin{table}[!t]
  ...
  \begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.34\columnwidth}
    ...
    Compute capacity & Enabled, with a 10 s scheduling window \\
    ...
    UAV memory scale & $\{0.6,0.7,\ldots,1.5\}$ \\
    Mid-feature scale & $\{0.4,0.6,\ldots,2.0\}$ \\
    Compute window & $\{2,3,\ldots,10\}$ s \\
    \addlinespace[1pt]
    \midrule
    ...
    Area size & Side length $\in\{240,500,1000\}$ m; $U=10$, $K=60$, $E=8$ \\
    Full-scale area & $(240,10,60,8)$, $(500,20,120,16)$, $(1000,40,240,24)$ for (m, $U$, $K$, $E$) \\
    Advantage scenario & $U=10$, $K=80$, $E=20$, memory scale $\{0.6,0.7,\ldots,1.2\}$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{itemize}
  ...
  \item \textbf{Importance}: 按专家的全局有效需求排序部署，忽略通信可达性、
        UAV 算力差异和网络位置收益。
...
Proposed 的平均总延迟为 $9892.6$\,ms，低于 Random Placement 的
$13036.6$\,ms 和 Importance 的 $12565.8$\,ms；与 Importance 相比，
Proposed 将平均延迟降低约 $21.3\%$。No-Sim 的平均总延迟达到
...
需求和拓扑信息，容易把专家部署到通信或计算条件不理想的 UAV 上；
Importance 只关注全局需求，不能区分专家部署在不同 UAV 上的通信收益；
No-Sim 则失去了相似专家带来的候选扩展和内存节省能力。
...
计算延迟为 $172.5$\,ms，低于 Random Placement 的 $234.6$\,ms、
Importance 的 $232.2$\,ms 和 No-Sim 的 $394.8$\,ms；平均传输延迟为
$39.9$\,ms，也低于 Random Placement 的 $56.2$\,ms 和 Importance 的
$46.9$\,ms。由于 No-Sim 在部分场景下无法完成覆盖预部署（原始专家覆盖
不可行），其传输延迟统计仅基于可行场景，本文不单独列出该项。
...
Proposed 平均产生 $114.4$ 次替代执行，高于 Random Placement 和
Importance；No-Sim 的替代次数为 $0$。这说明 SimAeroMoE 不仅在部署阶段
把相似专家作为候选，也在在线推理调度阶段实际利用这些替代专家来降低
传输和计算代价。
...
当 $\xi$ 提高时，低相似度替代被过滤，候选空间收缩。SimAeroMoE 在不同
$\xi$ 下保持稳定，说明通信感知评分和误差惩罚能够在候选数量与替代质量
之间形成较稳健的平衡。
...
当内存较紧时，部分方法会因为无法放置足够专家而不可行；在可行区域内，
No-Sim 和非通信感知方法也更容易受到专家覆盖不足和低质量部署位置的影响。
SimAeroMoE 通过覆盖预部署和相似替代优先保证原始需求可服务，再利用动态
贪心在剩余内存上补充高收益专家。随着内存容量增加，各方法均获得更多部署
空间，但 SimAeroMoE 仍保持较低延迟，表明其收益不仅来自部署更多专家，
也来自更合理的专家-UAV 匹配。
...
仅增加 UAV 数量，并保持任务和专家设置不变。随着 UAV 数量从 5 增加到
20，Proposed 的平均单任务延迟从约 $173$\,ms 降至约 $161$\,ms，并始终
低于所有基线。增加 UAV 会带来更多可部署位置和更短的平均接入距离，但
只有在部署策略能够同时利用任务需求、单跳连通性和 UAV 计算异构性时，
这些额外资源才能稳定转化为延迟收益。Random 和 Importance 在 UAV 较少
时容易受到部署位置不匹配影响；No-Sim 虽然也受益于更多 UAV，但由于缺乏
替代专家带来的内存弹性，延迟仍显著高于其他方法。
...
Fig.~\ref{fig:scale_task} 仅增加任务数量，并同时报告任务延迟和规划运行时间。
当任务数从 20 增加到 100 时，各方法的平均单任务延迟整体保持稳定，说明
任务数量增加主要改变需求统计和调度次数，而不会直接破坏单跳执行结构。
Proposed 在所有任务规模下保持最低延迟，表明需求驱动的有效专家建模能够
从更多任务样本中提取更稳定的部署偏好。另一方面，规划运行时间随任务数量
近似线性增加，这是因为需求统计、候选评分和逐层调度都需要遍历更多任务。
即便如此，所有方法的规划时间仍处于毫秒级，说明该部署流程适合准静态场景
中的周期性重规划。

\begin{figure}[!t]
  \centering
  \IfFileExists{photo/fig8_task_count.pdf}{%
    \includegraphics[width=\columnwidth]{photo/fig8_task_count.pdf}%
  }{%
    \fbox{\parbox{0.88\columnwidth}{\centering Missing figure file:\\
    photo/fig8\_task\_count.pdf}}%
  }
  \caption{任务数量变化下的延迟和规划时间。}
  \label{fig:scale_task}
\end{figure}

Fig.~\ref{fig:scale_expert} 进一步仅增加专家池规模。专家数量增加会扩大
候选部署空间，也会加重内存竞争。Proposed 在 8--24 个专家范围内保持最低
任务延迟和最高可行性，这是因为相似候选构造将新增专家转化为可替代资源，
而动态冗余惩罚避免把有限内存浪费在功能高度重叠的位置上。相比之下，
Random 和 Importance 在专家数量较大时更容易产生低收益部署，导致延迟升高
并在 24 个专家附近出现可行性下降。No-Sim 不使用替代专家，因此替代次数为
零；随着专家池变大，它反而更容易受到原始专家覆盖约束影响，在较大专家池
下不可行。规划运行时间方面，Proposed 随专家数量增加而上升，这是候选集合
和动态打分规模扩大带来的代价；但该代价仍保持在十几毫秒量级，低于准静态
部署重规划的时间尺度。

\begin{figure*}[!t]
  \centering
  \IfFileExists{photo/fig9_expert_count.pdf}{%
    \includegraphics[width=0.96\textwidth]{photo/fig9_expert_count.pdf}%
  }{%
    \fbox{\parbox{0.92\textwidth}{\centering Missing figure file:\\
...
系统资源。在固定资源设置下，区域从 $240\,\mathrm{m}\times240\,\mathrm{m}$
...
\begin{figure*}[!t]
  ...
  \IfFileExists{photo/fig10_area_scaling.pdf}{%
    \includegraphics[width=0.96\textwidth]{photo/fig10_area_scaling.pdf}%
  }{%
    \fbox{\parbox{0.92\textwidth}{\centering Missing figure file:\\
